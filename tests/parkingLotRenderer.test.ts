...
test('좌우 간격뷰의 시야 영역은 차량 중심축을 기준으로 서로 대칭이다', () => {
  const left = assistanceVisibilityPolygon(INITIAL_VEHICLE_STATE, 'left')
  const right = assistanceVisibilityPolygon(INITIAL_VEHICLE_STATE, 'right')

  left.forEach((point, index) => {
    assert.ok(Math.abs(point.x - right[index].x) < 0.01)
    assert.ok(Math.abs(
      (point.y + right[index].y) / 2 - INITIAL_VEHICLE_STATE.y,
    ) < 0.01)
  })
})
