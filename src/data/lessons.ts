...
export type MiniLesson = {
  ...
  title: string
  methods?: { id: string; title: string; steps: LessonStep[] }[]
  steps: LessonStep[]
}

export const LESSON_TRAJECTORIES = {
  approach: 'M70 90 H190',
  angle: 'M190 90 H178',
  reverseTurn: 'M178 90 C145.97 90 120 115.97 120 148',
  straightReverse: 'M120 148 V218',
...
export const LESSON_TRAJECTORY_GEOMETRY = {
  ...
  turnRadiusPixels: 58,
  entryStop: { x: 190, y: 90, headingDegrees: 0 },
  angleStop: { x: 178, y: 90, headingDegrees: 0 },
  alignedStop: { x: 120, y: 148, headingDegrees: -90 },
} as const

const TWO_BAY_SHOULDER_STEPS: LessonStep[] = [
  {
    title: '넓게 거리를 두고 평행 접근',
    description: '주차된 차량 앞쪽과 충분한 거리를 두고, 핸들을 중앙에 유지하며 천천히 전진합니다.',
    cue: '두 칸 옆 기준선에서 회전하려면 차량 앞쪽에 약 3m의 공간이 필요합니다.',
    durationSeconds: 9,
    gear: 'D',
    steering: '중앙',
    check: '차체 평행 · 핸들 중앙 · 앞쪽 회전 공간',
  },
  {
    title: '두 번째 차량 끝 선에 어깨 맞추기',
    description: '목표 칸 옆의 두 번째 차량까지 지나갑니다. 운전자 어깨가 그 차량의 바깥쪽 끝 주차선에 오면 완전히 정지합니다.',
    cue: '목표 칸의 끝 선이 아니라, 두 번째 차량 바깥쪽 선인지 확인하세요.',
    durationSeconds: 9,
    gear: 'D',
    steering: '중앙',
    check: '두 번째 차량 · 바깥쪽 끝 선 · 운전자 어깨',
  },
  {
  ...
  },
  {
    ...
    description: '천천히 곡선 후진합니다. 주차칸에 가까워지는 뒤 모서리와 바깥으로 도는 앞 모서리를 번갈아 확인합니다.',
  ...
  {
    title: '평행해지는 순간 정지하고 풀기',
    description: '차체가 주차선과 평행해지는 순간 완전히 정지하고 핸들을 중앙으로 풉니다.',
    cue: '깊이보다 차체가 평행해지는 시점을 먼저 확인하세요.',
    durationSeconds: 7,
    gear: 'R',
    steering: '중앙',
    check: '차체 평행 · 완전 정지 · 핸들 중앙',
  },
  {
    title: '좌우 간격을 보며 직선 후진',
    description: '양쪽 주차선 간격을 비교하며 천천히 직선 후진하고, 후방 거리를 확인해 마무리합니다.',
    cue: '좌우 위치는 간격뷰로, 뒤쪽 깊이는 후방 화면과 거리선으로 확인하세요.',
    durationSeconds: 11,
    gear: 'R',
    steering: '중앙',
    check: '좌우 간격 · 후방 거리 · 최종 정지',
  },
]
    ...
    methods: [
      {
        ...
        title: '어깨선 맞추고 진입',
        steps: [
          { title: '어깨를 먼 쪽 끝 선에 맞추기', description: '핸들을 중앙에 두고 주차칸과 약 1m 간격을 띄워 천천히 전진합니다. 운전자 어깨가 목표 주차칸의 먼 쪽 주차선에 오면 정지합니다.', cue: '차량 폭이 클수록 조금 더 띄우면 회전하기 수월합니다.', durationSeconds: 10, gear: 'D', steering: '중앙', check: '1m 간격 · 어깨선 일치 · 완전 정지' },
          { title: '주차 반대 방향으로 전진 조향', description: '주차할 칸의 반대 방향으로 핸들을 끝까지 꺾고 전진합니다. 사이드미러에 주차칸 안쪽 모서리가 보일 때 정지합니다.', cue: '너무 많이 나가면 반대편 차량이나 벽에 닿을 수 있으니 주의하세요.', durationSeconds: 12, gear: 'D', steering: '좌측 끝까지', check: '주차칸 안쪽 모서리 · 반대편 차 간격' },
          { title: '주차 방향으로 끝까지 조향', description: '완전히 정지한 상태에서 주차할 칸 방향으로 핸들을 끝까지 돌리고 후진 기어(R)로 변경합니다.', cue: '기어와 핸들 조작은 브레이크로 멈춘 상태에서 하세요.', durationSeconds: 8, gear: 'R', steering: '우측 끝까지', check: 'R 기어 · 주차 방향 최대 조향' },
          { title: '평행해질 때까지 곡선 후진', description: '핸들을 끝까지 돌린 상태로 천천히 후진합니다. 양쪽 사이드미러(간격뷰)를 번갈아 보며 간격을 확인하고 차체가 주차선과 평행해지면 정지합니다.', cue: '사이드미러를 양쪽 모두 확인하며 차가 일직선이 될 때 멈추세요.', durationSeconds: 14, gear: 'R', steering: '우측 끝까지', check: '좌측 ↔ 우측 간격뷰 · 차체 평행' },
          { title: '핸들을 풀고 깊이 맞추기', description: '차체가 주차선과 평행해지면 정지하고 핸들을 중앙으로 풉니다. 양쪽 선 간격을 유지하며 안전하게 끝까지 직선 후진합니다.', cue: '좌우 위치는 간격뷰로, 뒤쪽 깊이는 후방 화면과 거리선으로 확인하세요.', durationSeconds: 12, gear: 'R', steering: '중앙', check: '차체 평행 · 핸들 중앙 · 후방 거리' },
        ],
      },
      {
        id: '90-degree',
        title: '두 칸 옆 어깨선 후진 진입',
        steps: TWO_BAY_SHOULDER_STEPS,
      },
    ],
    steps: TWO_BAY_SHOULDER_STEPS,
  },
